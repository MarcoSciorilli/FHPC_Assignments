...
#pragma omp parallel reduction(+:th_avg_time) \
  ...
  {   int frac=N/nthreads;                                  // reductions: the one of runtime, which
      int resto=N-frac*nthreads;                            // happens in the whole parallel region;
      int start,stop;                                       // and the one on S, which takes place  
      int me = omp_get_thread_num();                        // in the for loop. 
      if(me<resto)
      {
        frac=frac+1;
        start=me*frac;
      ...
      {
        start=resto*(frac+1)+(me-resto)*frac;
      }
      stop=start+frac;
      // printf("%d %d \n",start,stop);                                                                                                       
